...
class WT_FMC_Renderer {
    ...
    constructor(fmc) {
        ...
        const oldPowerOn = this._fmc.onPowerOn.bind(this._fmc);
        const oldShutDown = this._fmc.onShutDown.bind(this._fmc);
        this._fmc.onPowerOn = ()=>{this._socket.setPower(true);this._socket.update();oldPowerOn();}
        this._fmc.onShutDown = ()=>{this._socket.setPower(false);this._socket.update();oldShutDown();}
        ...
        this._socket = new FWR_FMC_Websocket(this._fmc.onEvent.bind(this._fmc));
        this._socket.setPower(this._fmc.isStarted);
    }
    ...
    renderRow(template, rowId, dir = "left") {
      const row = this.getTRow(rowId);
      this.renderLetters(template, row, dir);
      this._socket.updateTemplateRow(
        rowId,
        [
          dir==="left" ? template : undefined,
          dir==="right" ? template : undefined,
          dir==="center" ? template : undefined,
        ]);
      this._socket.sendData();
    }
    ...
    onEvent(e) {
        ...
        if (document.getElementById("wt_container")) {
            ...
            // "this" is the _fmc here...
            const scratch = this.isDisplayingErrorMessage ? [this._errorMessage,"center"] : this.inOut;
            this._templateRenderer._socket.updateScratchpad(scratch);
            this._templateRenderer._socket.sendData();
        }
    }
    ...
    setMsg(msg) {
        ...
        this._socket.updateMessage(msg);
        this._socket.sendData();
    }
...
class FWR_FMC_Websocket {
    ...
    connectWebsocket(port) {
        ...
        this._socket.addEventListener('message', (event) => {
            ...
            if (msg.startsWith("event:")) {
                this.onEvent(`1_BTN_${msg.substring(6)}`);
    ...
    update(template, scratchpad, message) {
        this._template = template;
        this._scratchpad = scratchpad;
        this._message = message;
    }

    updateTemplateRow(rowId, template) {
      if (!template || !this._template) {
        return;
      }
      const oldRow = this._template[rowId];
      for (let i=0;i<3;i++) {
